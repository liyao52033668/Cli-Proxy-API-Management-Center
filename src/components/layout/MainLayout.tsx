...
const sidebarIcons: Record<string, ReactNode> = {
  ...
  codexInspection: <IconSidebarUsage size={18} />,
...
export function MainLayout() {
  ...
  const fullBrandName = 'Management Center';
  ...
  const navItems = [
    ...
    { path: '/codex-inspection', label: t('nav.codex_inspection'), icon: sidebarIcons.codexInspection },
  ...
  const getRouteOrder = (pathname: string) => {
    ...
    if (aiProvidersIndex !== -1) {
      ...
      if (normalizedPath.startsWith('/ai-providers/')) {
        ...
        if (normalizedPath.startsWith('/ai-providers/freebuff')) return aiProvidersIndex + 0.7;
