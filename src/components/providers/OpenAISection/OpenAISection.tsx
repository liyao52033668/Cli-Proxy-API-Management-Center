...
export function OpenAISection({
  ...
  const statusBarCache = useMemo(() => {
    ...
    configs.forEach((provider, index) => {
      ...
      const entries = (provider.apiKeyEntries || [])
        .map((entry) => ({ baseUrl: provider.baseUrl, apiKey: entry.apiKey }))
  ...
  const quotaTargets = useMemo(() => {
    ...
    configs.forEach((provider, index) => {
      const endpoint = resolveQuotaEndpoint(provider.quotaEndpoint, provider.baseUrl);
      if (!endpoint) return;
  ...
  const renderProviderCard = ({ config: provider, originalIndex }: IndexedOpenAIProvider) => {
    ...
    const quotaEndpoint = resolveQuotaEndpoint(provider.quotaEndpoint, provider.baseUrl);
