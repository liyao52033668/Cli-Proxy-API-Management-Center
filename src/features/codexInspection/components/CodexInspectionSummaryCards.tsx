...
export function CodexInspectionSummaryCards({ summary }: { summary: CodexInspectionSummary }) {
  ...
  const items = [
    {
      label: t('codex_inspection.total_files', { defaultValue: 'Total files' }),
      value: summary.totalFiles,
    },
    {
      label: t('codex_inspection.sampled_count', { defaultValue: 'Sampled' }),
      value: summary.sampledCount,
    },
    {
      label: t('codex_inspection.disabled_count', { defaultValue: 'Disabled' }),
      value: summary.disabledCount,
    },
    {
      label: t('codex_inspection.enabled_count', { defaultValue: 'Enabled' }),
    ...
    {
      label: t('codex_inspection.failed_count', { defaultValue: 'Inspection failed' }),
    ...
    >
      {items.map((item) => (
        <Card key={item.label} className="">
          <div style={itemStyle}>
            <div style={{ fontSize: 13, color: 'var(--text-secondary, #8b8b8b)' }}>
              {item.label}
            </div>
